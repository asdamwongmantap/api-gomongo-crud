...
func New(ctx context.Context, mongoConfig db.MongoConfig) (Mongo, error) {
	connPattern := "mongodb://%v:%v@%v:%v"
	if mongoConfig.Username == "" {
		connPattern = "mongodb://%s%s%v:%v"
	}

	clientUri := fmt.Sprintf(connPattern,
	...
	return NewWithURI(ctx, clientUri, mongoConfig.DBname)
}

func NewWithURI(ctx context.Context, uri, name string) (Mongo, error) {
	if uri == "" {
	...
	return &implementation{client, database}, nil
}

// NewMessageDB mongo db
func NewV2(ctx context.Context, mongoConfig db.MongoConfig) (Mongo, error) {
	driver := "mongodb"
	host := mongoConfig.Host
	port := mongoConfig.Port
	//username, err := url.QueryUnescape(mongoConfig.Username)
	//if err != nil {
	//	username = mongoConfig.Username
	//}
	//password, err := url.QueryUnescape(mongoConfig.Password)
	//if err != nil {
	...
	database := NewDatabase(client.Database(mongoConfig.DBname))
